...
/*  Decoder server code
* Identical code to enc_server, with adjustments to the decoding function
*/
...
/*
  ...
  Had to add a modulo function for negative numbers
*/
int modulo(int x,int N){
    return (x % N + N) %N;
}

char* decText(char arr[], char keyArr[], char theTable[]){
  int len = strlen(arr);
  ...
  for(int i = 0; i <len; i++){
    ...
    int total = tableIndex2 - tableIndex1; // ciphertext - key
    ...
    switch(spawnpid){
      ...
      case 0: // for successful forking we will execute child processes here
        ...
        if (connectionSocket < 0){
          ...
          encText = decText(plaintext, key, keyTable);
