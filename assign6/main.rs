...
fn main() {
    ...
    // creating two threads and joining them from 10_4 examples

    let xs_clone = xs.clone(); // create a clone of xs due to ownership conflicts
    let t1 = std::thread::spawn(move || map_data(&xs[0]));
    let t2 = std::thread::spawn(move || map_data(&xs_clone[1]));
    let _r1 = t1.join().unwrap();
    let _r2 = t2.join().unwrap();
    intermediate_sums.push(_r1); // push the finished threads onto the vector
    ...
    let mut handle_vec : Vec<JoinHandle<usize>> = Vec::new(); // This vector will hold all the threads being created
    for i in 0..num_partitions{ // spawn necessary threads
      let xs2_clone = xs2.clone();
      let handle = thread::spawn(move || (map_data(&xs2_clone[i])));
      handle_vec.push(handle); // store handles in our vector
    }

    // 4. Collects the intermediate sums from all the threads
    let mut intermediate_sums2 : Vec<usize> = Vec::new();
    for handle in handle_vec{ // gets values from handle and pushes to new intermediate_sums
      let a = handle.join().unwrap();
      intermediate_sums2.push(a);
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let size = v.len() / num_partitions; // divide the vector by num_partitions
    let extra = v.len() % num_partitions; // check if num_elements is a multiple of num_partitions
    let (v1, v2) = v.split_at(extra * (size + 1)); // split v at the index so that some partitions have one more
    // element that other partitions

    /* Here we will do the following:
        iterate through the v1, allocating the partitions as we go
        if v2 had partitions in its indices because num_elements was not a multiple of num_partitions,
        we will also iterate through v2
        link v2 back to v1
        since v1 is the last variable in this function, v1 will be returned
    */
    v1.chunks(size + 1)
        .chain(v2.chunks(size))
